...
class HopEntry:
    ...
    def _get_average_value(self, from_val: Union[str, float], to_val: Union[str, float]) -> float:
...
def analyze_brewing_parameters(hop_entries: List[HopEntry]) -> Dict:
    ...
    stats["averages"]["cohumulone"] = round(total_cohumulone / valid_cohumulone_count, 1) if valid_cohumulone_count > 0 else 0
...
def compare_hops_brewing_parameters(hop_entries: List[HopEntry]) -> Dict:
    ...
    if all(p == "Bittering" for p in purposes):
        comparison["recommendations"].append("All selected hops are primarily bittering hops - great for early boil additions")
    elif all(p == "Aroma" for p in purposes):
        comparison["recommendations"].append("All selected hops are aroma hops - ideal for late boil, whirlpool, or dry hop additions")
    else:
        comparison["recommendations"].append("Mix of hop purposes - consider using bittering hops early and aroma hops late in the boil")
